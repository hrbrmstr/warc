...
#' "\code{abmskrMVgu}" is taken from the defaults used by \code{wget} in
...
#' @note Only "\code{response}" or "\code{all}" are currently supported values for
#'   \code{warc_record_types}.
...
create_cdx <- function(warc_path, warc_record_types="response",
                       field_spec="abmskrMVgu", cdx_path) {

  if (grepl(".gz", warc_path)) {
    .Call('warc_int_create_cdx_from_gzwarc', PACKAGE = 'warc',
          warc_path=warc_path, warc_record_types=warc_record_types,
          field_spec=field_spec, cdx_prefix=cdx_path)
  } else {
    .Call('warc_int_create_cdx_from_warc', PACKAGE = 'warc',
          warc_path=warc_path, warc_record_types=warc_record_types,
          field_spec=field_spec, cdx_prefix=cdx_path)
  }
